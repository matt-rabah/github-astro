...
const getImageData = (
  ...
  if (url.startsWith(wixImagePrefix)) {
    ...
    const originWidth =
      imageProps.originWidth ||
      parseInt(searchParams.get("originWidth") || "0", 10);
    const originHeight =
      imageProps.originHeight ||
      parseInt(searchParams.get("originHeight") || "0", 10);
    if (originWidth && originHeight) {
      const uri = pathname.split("/").slice(2).join("/");
      const focalPoint =
        typeof imageProps.focalPointX === "number" &&
        typeof imageProps.focalPointY === "number"
          ? {
...
const ImageWrapper = forwardRef<HTMLSpanElement, ImageWrapperProps>(
  ({ data, className, style, children }, ref) => {
...
const WixImage = forwardRef<HTMLImageElement, WixImageProps>(
  ({ data, fittingType = "fill", className, style, ...props }, parentRef) => {
    ...
    if (size) {
      ...
      const targetHeight =
        size.height || height * (size.width / width) || height;
      const targetWidth = size.width || width * (size.height / height) || width;
      const transformOptions: ImageTransformOptions = focalPoint
        ? { focalPoint }
        : undefined;
      imgProps.src = scale(
        data.id,
      ...
      const { uri, ...placeholder } = getPlaceholder(
        fittingType ?? "fit",
        data,
        { htmlTag: "img" },
      );
